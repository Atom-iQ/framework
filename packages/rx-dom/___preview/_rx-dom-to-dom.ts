import { _createRxDOM } from './_ts_components_to_rx_dom';
import {App, mS} from './_tsx-to-ts';
...
const RHS = new BehaviorSubject('Rx UI Suite');
const RSFS = new BehaviorSubject(true);

const __rootComponentState = {
  header: RHS.asObservable(),
  setHeader: (header: string) => RHS.next(header),
  showFooter: RSFS.asObservable(),
  setShowFooter: (show: boolean) => RSFS.next(show)
};

const firstHeaderRxNode = {
  type: 'header',
  props: {},
  children: [
    of({
...
/*
  ...
  renderRxDom({
    type: 'main',
    props: {
      className: of('Rx-dom') // Observable property,
      id: 'main',
  ...
  -> map rxDom children to dom children:
    - subscribe to observable children (all _jsx calls returning
      observable) - call renderRxDom for child on every new child value
...
/*
  ...
  -> set static class as 'Rx-dom__section'
...
/*
  ...
  <div id="root">
    <main id="main" class="Rx-dom"> // attached listener to 'click'
      <section class="Rx-dom__section"></section>
    </main>
  </div>
 */
...
/*
  ...
  -> insert text node to 'Rx-dom__h1'
...
/*
  11. Final generated dom (after 'h2' text child is inserted)
    <div id="root">
      <main id="main" class="Rx-dom"> // attached listener to 'click'
        <header>
          <h1 class="Rx-dom__h1>Rx UI Suite WOW!</h1>
        </header>
        <section class="Rx-dom__section">
          <header>
            <h2 class="Rx-dom__h2>New Section WOW!</h1>
          </header>
        </section>
        <footer>
          <button>Footer</button> // attached listener to 'click'
...
/*
  ...
  -> the only one element that is changing is 'Rx UI Suite WOW!'
