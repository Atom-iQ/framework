...
export const combineMiddlewares: CombineMiddlewaresFn = (
  ...
  const addMiddlewareFromPackage: AddMiddlewareFactory = (pkgName: string) => (
    ...
    if (middlewareType === 'component') {
      ...
      combinedMiddlewares.component[middleware.alias || name] = middleware.fn as ComponentMiddleware
    } else {
      ...
      combinedMiddlewares[middlewareType].order = combinedMiddlewares[middlewareType].order.concat(
        name
      )
    }
  }
