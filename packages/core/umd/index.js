(function (global, factory) {
    ...
    (global = typeof globalThis !== 'undefined' ? globalThis : global || self, factory(global.iQ = {}, global.rxjs, global.rxjs.operators));
    ...
    const connectCssProperties = (styles, element, propsSubscription) => {
        Object.entries(styles).forEach(([cssPropName, cssPropValue]) => {
            const parsedCssPropName = cssPropName.includes('-') ?
                ...
                cssPropName;
            if (rxjs.isObservable(cssPropValue)) {
    ...
    const connectStyleProp = (rvdElement, element, propsSubscription) => (propName, propValue) => {
        if (rxjs.isObservable(propValue)) {
    ...
    const connectEventProp = (rvdElement, element, propsSubscription) => (propName, propValue) => {
        if (isEventHandler(propName)) {
            if (isRxEventHandler(propName)) {
                ...
                const event$ = operators.map(event => Object.assign({ element }, event))(rxjs.fromEvent(element, eventName));
    ...
    function connectElementProps(rvdElement, element) {
        const propsSubscription = new rxjs.Subscription();
        if (rvdElement.props) {
    ...
    function renderRvdComponent(componentIndex, element, createdChildren, childrenSubscription, renderNewCallback) {
        return (rvdComponentElement) => {
            ...
            if (rxjs.isObservable(componentChild)) {
    ...
    const renderChildren = (children, element) => {
        const childrenSubscription = new rxjs.Subscription();
    ...
    function renderRootChild(rootRvdElement, rootDOMElement) {
         ...
         * props subscriptions.
         */
        const rootSubscription = new rxjs.Subscription();
        /**
    ...
    const createState = (initialState) => {
        const stateSubject = new rxjs.BehaviorSubject(initialState);
        const state$ = stateSubject.asObservable();
        const nextState = valueOrCallback => {
            if (isFunction(valueOrCallback)) {
                operators.first()(state$).subscribe(valueOrCallback);
            }
            else {
    ...
    const eventState = (operator) => {
         ...
         * to new observers.
         */
        const stateSubject = new rxjs.ReplaySubject(1);
        const state$ = stateSubject.asObservable();
        /**
        ...
        const connectEvent = preOperator => (event$) => {
            const source$ = operator
                ...
                : preOperator
                    ? preOperator(event$)
                    : event$;
            return rxjs.pipe(operators.tap(stateSubject.next), operators.catchError(error => {
    ...
    exports.createElement = createRvdElement;
    exports.createState = createState;
    exports.eventState = eventState;
    exports.iQRvdStart = createRvDOM;
    exports.useState = createState;
