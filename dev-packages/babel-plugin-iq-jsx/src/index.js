...
module.exports = function () {
  ...
  const initConfig = path => {
    ...
    const jsxObjectTransformer = (type, props, children) => {
      return t.callExpression(moduleName, [type, props, children])
    }

    const importDeclaration = t.importDeclaration(
      [t.importSpecifier(moduleName, moduleName)],
      t.stringLiteral('@atom-iq/core/old_lib/jsx')
    )
  ...
  const visitJSXElement = (path, state) => {
    ...
    const JSXElementName = transformOnType({
      JSXIdentifier: node =>
        variablesRegex.test(node.name) ? t.identifier(node.name) : JSXIdentifier(node),
      JSXNamespacedName,
      JSXMemberExpression
    })
    ...
    const JSXElement = node => {
      ...
      const props = isFragment
        ...
        : JSXAttributes(
            childrenInProps
              ? node.openingElement.attributes.filter(attribute => {
                  const attributeName = JSXAttributeName(attribute.name)
                  if (attributeName && attributeName.value) {
                    return attributeName.value !== 'children'
                  }
      ...
      const elementHasChildren = node.closingElement || childrenInProps || isFragment

      const children = elementHasChildren ? JSXChildren(getChildren()) : t.nullLiteral()

      return jsxObjectTransformer(type, props, children)
    }
