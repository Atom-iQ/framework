...
var babelSettings = {
  ...
  plugins: [[plugin, { imports: true, defineAllArguments: false }], '@babel/plugin-syntax-jsx']
}
...
describe('Transforms', function () {
  ...
  describe('Dynamic children', function () {
    ...
    it('Should add normalize call when there is dynamic and static children mixed', function () {
      expect(transform('<div>{a}<div>1</div></div>')).to.equal(
    ...
    it('Should not add normalize call when all children are known', function () {
      expect(transform('<div><FooBar/><div>1</div></div>')).to.equal(
    ...
    it('Should create textVNodes when there is no normalization needed and its multiple children', function () {
      expect(transform('<div><FooBar/>foobar</div>')).to.equal(
    ...
    it('Should mark parent vNode with $HasKeyedChildren if no normalize is needed and all children are keyed', function () {
      expect(transform('<div><FooBar key="foo"/><div key="1">1</div></div>')).to.equal(
  ...
  describe('different types', function () {
    ...
    it('Should transform media', function () {
      expect(transform('<media>foobar</media>')).to.equal(
        'createVNode(1, "media", null, "foobar", 16);'
      )
    })

    it('Should transform textarea', function () {
      expect(transform('<textarea>foobar</textarea>')).to.equal(
  ...
  describe('Special flags', function () {
    it('Should add keyed children flag', function () {
      expect(transform('<div $HasKeyedChildren>{magic}</div>')).to.equal(
        'createVNode(1, "div", null, magic, 8);'
      )
    })

    it('Should not normalize if noNormalize set', function () {
      expect(transform('<div $HasVNodeChildren>{magic}</div>')).to.equal(
    ...
    it('Should set hasTextChildren flag and not create textVNode when $HasTextChildren is used ( dynamic )', function () {
      expect(transform('<div $HasTextChildren>{foobar}</div>')).to.equal(
    ...
    it('Should set hasTextChildren flag and not create textVNode when $HasTextChildren is used ( hardcoded )', function () {
      expect(transform('<div $HasTextChildren>text</div>')).to.equal(
    ...
    it('Should set hasTextChildren flag and not create textVNode when $HasTextChildren is used ( hardcoded ) #2', function () {
      expect(transform('<div $HasTextChildren>{"testing"}</div>')).to.equal(
    ...
    it('Should add non keyed children flag', function () {
      expect(transform('<div $HasNonKeyedChildren>{test}</div>')).to.equal(
  ...
  describe('spreadOperator', function () {
    it('Should add call to normalizeProps when spread operator is used', function () {
      expect(transform('<div {...props}>1</div>')).to.equal(
    ...
    it('Should add call to normalizeProps when spread operator is used #2', function () {
      expect(transform('<div foo="bar" className="test" {...props}/>')).to.equal(
    ...
    it('Should add call to normalizeProps when spread operator is used inside children for Component', function () {
      expect(transform('<FooBar><BarFoo {...props}/><NoNormalize/></FooBar>')).to.equal(
  ...
  describe('Basic scenarios', function () {
    ...
    it('className should be in third parameter as string when its element', function () {
      expect(transform('<div className="first second">1</div>')).to.equal(
        'createVNode(1, "div", "first second", "1", 16);'
      )
    })

    it('className should be in fifth parameter as string when its component', function () {
      expect(transform('<UnknownClass className="first second">1</UnknownClass>')).to.equal(
        'createComponentVNode(2, UnknownClass, {\n  "className": "first second",\n  children: "1"\n});'
      )
    })

    it('JSXMemberExpressions should work', function () {
      expect(transform('<Components.Unknown>1</Components.Unknown>')).to.equal(
        'createComponentVNode(2, Components.Unknown, {\n  children: "1"\n});'
      )
    })

    it('class should be in third parameter as variable', function () {
      expect(transform('<div class={variable}>1</div>')).to.equal(
    ...
    it('Should call createVNode twice and text children', function () {
      ...
      ).to.equal('createVNode(1, "div", null, createVNode(1, "div", null, "single", 16), 2);')
    })

    it('Events should be in props', function () {
      expect(transform('<div id="test" onClick={func} class={variable}>1</div>')).to.equal(
        'createVNode(1, "div", variable, "1", 16, {\n  "id": "test",\n  "onClick": func\n});'
      )
    })
    ...
    it('Should transform input and htmlFor correctly', function () {
      ...
      var expected =
        'createVNode(1, "label", null, createVNode(64, "input", null, null, 1, {\n  "id": id,\n  "name": name,\n  "value": value,\n  "onChange": onChange,\n  "onInput": onInput,\n  "onKeyup": onKeyup,\n  "onFocus": onFocus,\n  "onClick": onClick,\n  "type": "number",\n  "pattern": "[0-9]+([,.][0-9]+)?",\n  "inputMode": "numeric",\n  "min": minimum\n}), 2, {\n  "for": id\n});'
      expect(result).to.equal(expected)
    })

    it('Should transform onDoubleClick to native html event', function () {
      expect(transform('<div onDoubleClick={foobar}></div>')).to.eql(
  ...
  describe('contenteditbale', function () {
    it('Should set additional byte on when contenteditbale attribute is found', function () {
      expect(transform('<div contentEditable></div>')).to.eql(
        'createVNode(4097, "div", null, null, 1, {\n  "contentEditable": true\n});'
      )
      expect(transform('<span contenteditable="false"></span>')).to.eql(
        'createVNode(4097, "span", null, null, 1, {\n  "contenteditable": "false"\n});'
      )
      expect(transform('<div contenteditable></div>')).to.eql(
        'createVNode(4097, "div", null, null, 1, {\n  "contenteditable": true\n});'
      )
      expect(transform('<div contentEditable={logic}></div>')).to.eql(
        'createVNode(4097, "div", null, null, 1, {\n  "contentEditable": logic\n});'
      )
      expect(transform('<div contentEditable="true"></div>')).to.eql(
        'createVNode(4097, "div", null, null, 1, {\n  "contentEditable": "true"\n});'
      )
    })
  })
  ...
  describe('Pragma option', function () {
    var babelSettingsPragma = {
      ...
      plugins: [[plugin, { imports: false, pragma: 't.some' }], '@babel/plugin-syntax-jsx']
    }
  ...
  describe('defineAllArguments option', function () {
    var babelSettingsPragma = {
      ...
      plugins: [[plugin, { imports: false, defineAllArguments: true }], '@babel/plugin-syntax-jsx']
    }
    ...
    it('Should replace createVNode to pragma option value', function () {
      expect(pluginTransformAllArgs('<div></div>')).to.equal(
  ...
  describe('SVG attributes React syntax support', function () {
    it('Should transform xlinkHref to xlink:href', function () {
      expect(transform('<svg><use xlinkHref="#tester"></use></svg>')).to.equal(
        'createVNode(32, "svg", null, createVNode(32, "use", null, null, 1, {\n  "xlink:href": "#tester"\n}), 2);'
      )
    })

    it('Should transform strokeWidth to stroke-width', function () {
      expect(transform('<svg><rect strokeWidth="1px"></rect></svg>')).to.equal(
        'createVNode(32, "svg", null, createVNode(32, "rect", null, null, 1, {\n  "stroke-width": "1px"\n}), 2);'
      )
    })

    it('Should transform strokeWidth to stroke-width', function () {
      expect(transform('<svg><rect fillOpacity="1"></rect></svg>')).to.equal(
        'createVNode(32, "svg", null, createVNode(32, "rect", null, null, 1, {\n  "fill-opacity": "1"\n}), 2);'
      )
    })
  })
  ...
  describe('text node and elements mixed', () => {
    it('Should createTextVNode when there are siblings', () => {
      expect(transform('<div>Okay<span>foo</span></div>')).to.eql(
    ...
    it('Should createTextVNode when text node is under short syntax fragment', () => {
      expect(transform('<>Okay<span>foo</span></>')).to.eql(
    ...
    it('Should handle many dynamic and non dynamic children short syntax', () => {
      expect(transform('<><><span></span>Text{Wohoo}</></>')).to.eql(
    ...
    it('Should always keep text node as children even if there is one when parent is long syntax Fragment', () => {
      expect(transform('<Fragment><Fragment>Text</Fragment></Fragment>')).to.eql(
        'createFragment([createFragment([createTextVNode("Text")], 4)], 4);'
      )
    })

    it('Should createTextVNode when text node is under large syntax fragment', () => {
      expect(transform('<Fragment>Okay<span>foo</span></Fragment>')).to.eql(
        'createFragment([createTextVNode("Okay"), createVNode(1, "span", null, "foo", 16)], 4);'
      )
    })
    ...
    it('Should always long syntax Fragment', () => {
      expect(transform('<Fragment><Fragment><div>Text</div></Fragment></Fragment>')).to.eql(
        'createFragment([createFragment([createVNode(1, "div", null, "Text", 16)], 4)], 4);'
      )
    })

    it('Should handle many dynamic children long syntax', () => {
      expect(transform('<Fragment><Fragment>{Frag}Text{Wohoo}</Fragment></Fragment>')).to.eql(
        'createFragment([createFragment([Frag, createTextVNode("Text"), Wohoo], 0)], 4);'
      )
    })
    ...
    it('Should handle many dynamic and non dynamic children long syntax', () => {
      expect(
        transform('<Fragment><Fragment><span></span>Text{Wohoo}</Fragment></Fragment>')
      ).to.eql(
        'createFragment([createFragment([createVNode(1, "span"), createTextVNode("Text"), Wohoo], 0)], 4);'
      )
    })
  ...
  describe('Imports', function () {
    it('Should not fail if createVNode is already imported', function () {
      expect(pluginTransform('import {createVNode} from "inferno"; var foo = <div/>;')).to.equal(
        'import { createVNode } from "inferno";\nvar foo = createVNode(1, "div");'
      )
    })

    it('Should add import to createVNodeComponent but not to createVNode if createVNode is already delcared', function () {
      expect(pluginTransform('import {createVNode} from "inferno"; var foo = <FooBar/>;')).to.equal(
  ...
  describe('Children', function () {
    it('Element Should prefer child element over children props', function () {
      expect(transform('<div children="ab">test</div>')).to.eql(
        'createVNode(1, "div", null, "test", 16);'
      )
    })

    it('Element Should prefer prop over empty children', function () {
      expect(transform('<div children="ab"></div>')).to.eql(
    ...
    it('Component Should prefer child element over children props', function () {
      expect(transform('<Com children="ab">test</Com>')).to.eql(
        'createComponentVNode(2, Com, {\n  children: "test"\n});'
      )
    })

    it('Component Should prefer prop over empty children', function () {
      expect(transform('<Com children="ab"></Com>')).to.eql(
        'createComponentVNode(2, Com, {\n  "children": "ab"\n});'
      )
    })

    it('Component Should use prop if no children exists', function () {
      expect(transform('<Com children="ab"/>')).to.eql(
        'createComponentVNode(2, Com, {\n  "children": "ab"\n});'
      )
    })
    ...
    it('Component should create vNode for children', function () {
      expect(transform('<Com children={<div>1</div>}/>')).to.eql(
        'createComponentVNode(2, Com, {\n  "children": createVNode(1, "div", null, "1", 16)\n});'
      )
    })

    it('Should prefer xml children over props', function () {
      expect(transform('<foo children={<span>b</span>}></foo>')).to.eql(
  ...
  describe('Fragments', function () {
    describe('Short syntax', function () {
      ...
      it('Should createFragment keyed children', function () {
        expect(transform('<><span key="ok">kk</span><div key="ok2">ok</div></>')).to.eql(
      ...
      it('Should createFragment non keyed children', function () {
        expect(transform('<><div>1</div><span>foo</span></>')).to.eql(
    ...
    describe('Long syntax', function () {
      describe('Fragment', function () {
        ...
        it('Should createFragment', function () {
          expect(transform('<Fragment>Test</Fragment>')).to.eql(
        ...
        it('Should createFragment non keyed children', function () {
          expect(transform('<Fragment><div>1</div><span>foo</span></Fragment>')).to.eql(
            'createFragment([createVNode(1, "div", null, "1", 16), createVNode(1, "span", null, "foo", 16)], 4);'
          )
        })
        ...
        it('Should createFragment non keyed children', function () {
          expect(transform('<Fragment key="foo"><div>1</div><span>foo</span></Fragment>')).to.eql(
        ...
        // Optimization flags
        it('Should createFragment non keyed children', function () {
          expect(transform('<Fragment key="foo" $HasKeyedChildren>{magic}</Fragment>')).to.eql(
            'createFragment(magic, 8, "foo");'
          )
        })

        it('Should createFragment non keyed children', function () {
          expect(transform('<Fragment key="foo" $HasNonKeyedChildren>{magic}</Fragment>')).to.eql(
      ...
      describe('Inferno.Fragment', function () {
        it('Should createFragment', function () {
          expect(transform('<Inferno.Fragment>Test</Inferno.Fragment>')).to.eql(
            'createFragment([createTextVNode("Test")], 4);'
          )
        })

        it('Should createFragment dynamic children', function () {
          expect(transform('<Inferno.Fragment>{dynamic}</Inferno.Fragment>')).to.eql(
        ...
        it('Should createFragment non keyed children', function () {
          expect(
            transform('<Inferno.Fragment><div>1</div><span>foo</span></Inferno.Fragment>')
          ).to.eql(
            'createFragment([createVNode(1, "div", null, "1", 16), createVNode(1, "span", null, "foo", 16)], 4);'
          )
        })
        ...
        it('Should createFragment non keyed children', function () {
          expect(
            transform('<Inferno.Fragment key="foo"><div>1</div><span>foo</span></Inferno.Fragment>')
          ).to.eql(
            'createFragment([createVNode(1, "div", null, "1", 16), createVNode(1, "span", null, "foo", 16)], 4, "foo");'
          )
        })
        ...
        it('Should ignore all other props', function () {
          expect(
            transform(
              '<Inferno.Fragment abc="foobar" id="test" key="foo"><div>1</div><span>foo</span></Inferno.Fragment>'
            )
          ).to.eql(
            'createFragment([createVNode(1, "div", null, "1", 16), createVNode(1, "span", null, "foo", 16)], 4, "foo");'
          )
        ...
        it('Should createFragment non keyed children', function () {
          expect(
            transform('<Inferno.Fragment key="foo" $HasKeyedChildren>{magic}</Inferno.Fragment>')
          ).to.eql('createFragment(magic, 8, "foo");')
        })

        it('Should createFragment non keyed children', function () {
          expect(
            transform('<Inferno.Fragment key="foo" $HasNonKeyedChildren>{magic}</Inferno.Fragment>')
          ).to.eql('createFragment(magic, 4, "foo");')
        })
      })
    })
  })
