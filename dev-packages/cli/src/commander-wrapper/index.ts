...
module.exports = (mainCommandName: string): CommanderWrapper => {
  if (!program) {
    program = new commander.Command(mainCommandName)
  }

  const declareCommand =
    (command: Command) =>
    (...params: ((cmd?: Command) => Command)[]): Command =>
      params.reduce((cmd, param) => param(cmd), command)

  const subCommand =
    (cmd: string, description?: string, config?: unknown) =>
    (parent: Command): Command =>
      parent.command(cmd, description, config)
